  ...
  _createClass(CustomExport, [{
    ...
    value: function generateExportStack(_ref) {
      ...
      var scaleBarImage = apiGenerators.scaleBar().then(function (data) {
        return {
          graphic: data.graphic,
          offset: [mapImageSize.width - 10 - 120, mapImageSize.height - 30 - 10]
        };
      ...
      var datasets = {
        ahccd: "Données climatiques historiques ajustées",
        cangrd: "Variations de température et de précipitations",
        capa: "Estimations de précipitations",
      ...
      if (['cmip5','dcs'].indexOf(dataset) !== -1) {
        var descriptionImage = apiGenerators.footNote('Les changements projetés sont relatives à la moyenne de 1986-2005.').then(function (data) {
          return {
            graphic: data.graphic,
        ...
        var timeSlice = new RegExp('[?&]ts=([0-9])').exec(window.location.href)[1];
        if (timeSlice) {
        ...
        var timePeriodImage = new Promise(function(resolve, reject) {
          legendImage.then(function (promiseResult) {
            apiGenerators.htmlMarkup("<span><b>Période : </b> ".concat(slices[parseInt(timeSlice)], "</span>")).then(function (data) {
              resolve({
                graphic: data.graphic,
        ...
        var rcpImage = new Promise(function(resolve, reject) {
          legendImage.then(function (promiseResult) {
            apiGenerators.htmlMarkup("<span><b>Scénarios : </b>".concat(scenarios[rcp], "</span>")).then(function (data) {
              resolve({
                graphic: data.graphic,
      ...
      if (dataset !== 'hydro' && dataset !== 'capa') {
        ...
        var timeOfYearImage = new Promise(function(resolve, reject) {
          legendImage.then(function (promiseResult) {
            apiGenerators.htmlMarkup("<span><b>Temps de l'année : </b> ".concat(times[timePeriod], '</span>')).then(function (data) {
              resolve({
                graphic: data.graphic,
      ...
      if (dataset === 'capa') {
        ...
        var dateImage = new Promise(function (resolve, reject) {
          legendImage.then(function (promiseResult) {
            apiGenerators.htmlMarkup("<span style=\"width: 150px; position = absolute;\"><b>Période : </b> ".concat(months[date.getUTCMonth()], ' ', date.getUTCDate(), ', ', date.getUTCFullYear(), ', ', date.getUTCHours(), ':00 UTC', "</span>")).then(function (data) {
              resolve({
                graphic: data.graphic,
                offset: [mapImageSize.width + 30, promiseResult.graphic.height + 40]
              });
